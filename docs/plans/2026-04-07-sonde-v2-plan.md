...
**Goal:** Ground-up rebuild of Sonde as a zero-friction, browser-based HTTP latency diagnostic tool with meteorological instrument aesthetic, two-tier measurement, and shareable results.
...
- [ ] **1.1 — Scaffold the project**
  ...
  <html lang="en">
    <head>
      ...
      <title>Sonde — HTTP Latency Diagnostic</title>
...
- [ ] **10.2 — Implement `src/lib/utils/persistence.ts`**
  ...
  export function loadPersistedSettings(): PersistedSettings | null {
    try {
      ...
      console.warn('[sonde] Failed to load persisted settings:', err);
      return null;
    }
  }
  ...
  export function saveSettings(settings: PersistedSettings): void {
    try {
      ...
      console.warn('[sonde] Failed to save settings:', err);
    }
  }
...
describe('buildShareURL / parseShareURL', () => {
  it('builds a URL with a hash fragment', () => {
    const url = buildShareURL('https://sonde.example.com', BASE_CONFIG_PAYLOAD);
  ...
  it('parseShareURL returns null when hash is absent', () => {
    expect(parseShareURL('https://sonde.example.com/')).toBeNull();
  });

  it('parseShareURL returns null for a hash with no s= param', () => {
    expect(parseShareURL('https://sonde.example.com/#unrelated=abc')).toBeNull();
  });
});
```
...
{#if open}
  ...
  <div
    ...
    role="dialog"
    aria-modal="true"
    aria-label="Share Sonde results"
    tabindex="-1"
  >
...
export default [
  // ...existing config...
  {
    ...
    rules: {
      'sonde/no-raw-visual-values': 'error',
    },
