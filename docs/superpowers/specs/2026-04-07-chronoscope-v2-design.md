...
# Design Specification -- Chronoscope v2
...
**AC5: Performance and Accessibility.** When Chronoscope is loaded on a cold cache over a 3G Fast connection, Lighthouse reports Performance >= 95 and Accessibility >= 90. All interactive elements are reachable and operable via keyboard. Color encoding passes WCAG AA contrast against the background surface, verified across protanopia, deuteranopia, and tritanopia simulations.
...
| **Alerting / continuous monitoring** | Chronoscope is a diagnostic tool you reach for, not a monitoring platform. Deliberate positioning. |
...
The name "Chronoscope" (radiochronoscope -- a meteorological probe) drives the visual language. Colors derive from weather radar, not startup gradients. The palette is perceptually uniform and CVD-safe (colorblind-safe), based on Viridis principles rather than HSL rotation.
...
| Page title | `typography.title.*` | Inter | 24px | 700 | 1.2 | "Chronoscope" branding in header |
...
The defining motion of Chronoscope. When a measurement completes, a concentric ring emanates from the data point on the effects canvas.
...
Chronoscope is dark-only in v1. There is no light theme. The dark theme is not "inverted light" -- it is the primary design.
...
Chronoscope has no accounts, sessions, tokens, or API keys. There is nothing to authenticate and nothing to misconfigure. This is by design -- zero-friction access is a core value proposition.

### Attack Surface Analysis

**DDoS via Chronoscope:**
- Risk: Using Chronoscope to flood a target with requests.
- Mitigation: Sequential rounds (not parallel floods), configurable delay between rounds (default 1000ms), per-endpoint request cap. At maximum speed with 0ms delay, Chronoscope sends one GET request per round per endpoint -- less traffic than loading a typical webpage. Not a meaningful DDoS vector.
...
- Mitigation: This is standard browser behavior, not unique to Chronoscope. The `no-cors` fetch mode means the browser controls access, not the tool. The EndpointPanel includes a subtle note: "Requests are sent from your browser" -- communicating that private addresses are accessed from the user's local network.
...
- **Greenfield on fork:** Chronoscope v2 is a ground-up rebuild on a forked repo. No existing Chronoscope v2 users. The original s80 users are unaffected (different repo/URL).
...
*This specification covers the complete Chronoscope v2 design. Implementation proceeds via the Main-Thread Layered Canvas approach with the component hierarchy, rendering pipeline, and quality infrastructure described above. The measurement schema is designed to be telemetry-ready for the Layer 4 vision (collective edge intelligence) without building telemetry transmission in v1.*
