...
# Acceptance Criteria — Chronoscope v2

## AC1: Instant Comparative Diagnosis
When a user opens Chronoscope and enters 2+ endpoint URLs, live latency data for all endpoints appears on a synchronized timeline within 5 seconds of starting the test, with each endpoint visually distinguishable by color and accompanied by real-time p50 latency displayed in the summary area.
...
When Chronoscope is loaded on a cold cache over a 3G Fast connection, Lighthouse reports Performance >= 95 and Accessibility >= 90. All interactive elements are reachable and operable via keyboard. Color encoding passes WCAG AA contrast against the background surface, verified across protanopia, deuteranopia, and tritanopia simulations.
